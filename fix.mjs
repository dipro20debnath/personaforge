#!/usr/bin/env node
// ═══════════════════════════════════════════════════
//  PersonaForge Fix — replaces better-sqlite3 with
//  sql.js (pure JS, zero native build needed)
//  Run:  node fix.mjs
// ═══════════════════════════════════════════════════
import { writeFileSync, rmSync, existsSync } from "fs";
import { join } from "path";

const file = (p, c) => writeFileSync(join(".", p), c);

// ─── Clean old broken installs ───
console.log("🧹 Cleaning old installs...");
try { if(existsSync("server/node_modules")) rmSync("server/node_modules", { recursive:true, force:true }); } catch(e) { console.log("   ⚠ Could not auto-delete server/node_modules — delete it manually if install fails"); }
try { if(existsSync("server/personaforge.db")) rmSync("server/personaforge.db", { force:true }); } catch {}
try { if(existsSync("node_modules")) rmSync("node_modules", { recursive:true, force:true }); } catch {}
...
// ─── 2. Fix server/package.json (sql.js replaces better-sqlite3) ───
file("server/package.json", JSON.stringify({
  name: "personaforge-server",
  type: "module",
...
// ─── 3. Rewrite server/src/db.js for sql.js ───
file("server/src/db.js", `import initSqlJs from 'sql.js';
