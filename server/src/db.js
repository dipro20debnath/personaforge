...
try {
  const existingDemo = await db.prepare('SELECT id FROM users WHERE email = ?').get('demo@personaforge.com');
  if (!existingDemo) {
    const { v4: uuid } = await import('uuid');
    ...
    await db.prepare('INSERT INTO users (id, email, password, status) VALUES (?, ?, ?, ?)').run(demoUserId, 'demo@personaforge.com', hashedPassword, 'active');

    // Create profile (78% of fields: 7/9 fields filled)
    await db.prepare('INSERT INTO profiles (user_id, display_name, bio, avatar_url, locale, timezone, dob, country, xp, level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(demoUserId, 'Alex Johnson', 'Personal growth enthusiast | 100-day journey', 'https://i.pravatar.com/150?img=33', 'en', 'America/New_York', '1995-06-15', 'United States', 4850, 6);

    // Create assessment (personality profile data)
    await db.prepare('INSERT INTO assessments (id, user_id, openness, conscientiousness, extraversion, agreeableness, neuroticism, responses) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(uuid(), demoUserId, 0.78, 0.85, 0.72, 0.81, 0.42, '{}');
    ...
    for (const goal of goals) {
      const goalId = uuid();
      goalIds.push(goalId);
      await db.prepare('INSERT INTO goals (id, user_id, title, description, metric, target_value, current_value, status, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(goalId, demoUserId, goal.title, goal.desc, goal.metric, goal.target, goal.current, goal.status, 'personal');
    }

    // Create goal milestones
    await db.prepare('INSERT INTO goal_milestones (id, goal_id, title, done) VALUES (?, ?, ?, ?)').run(uuid(), goalIds[0], 'Complete 10K', 1);
    await db.prepare('INSERT INTO goal_milestones (id, goal_id, title, done) VALUES (?, ?, ?, ?)').run(uuid(), goalIds[0], 'Complete Half-Marathon', 1);
    await db.prepare('INSERT INTO goal_milestones (id, goal_id, title, done) VALUES (?, ?, ?, ?)').run(uuid(), goalIds[0], 'Train with running club', 0);
    ...
    for (const habit of habits) {
      ...
      await db.prepare('INSERT INTO habits (id, user_id, title, cadence, streak, best_streak) VALUES (?, ?, ?, ?, ?, ?)').run(habitId, demoUserId, habit.title, habit.cadence, habit.streak, habit.best);
    }
    ...
    for (let i = 0; i < 30; i++) {
      ...
      await db.prepare('INSERT INTO habit_checkins (id, habit_id, day, done) VALUES (?, ?, ?, ?)').run(uuid(), habitIds[0], day, done);
    }
    ...
    for (const skill of skills) {
      await db.prepare('INSERT INTO skills (id, user_id, name, category, self_level, target_level, world_avg) VALUES (?, ?, ?, ?, ?, ?, ?)').run(uuid(), demoUserId, skill.name, skill.category, skill.self, skill.target, skill.world);
    }
    ...
    for (let i = 0; i < 5; i++) {
      await db.prepare('INSERT INTO learning_enrollments (id, user_id, path_id, progress) VALUES (?, ?, ?, ?)').run(uuid(), demoUserId, learningPaths[i], progresses[i]);
    }
    ...
    await db.prepare('INSERT INTO daily_routines (id, user_id, routine) VALUES (?, ?, ?)').run(uuid(), demoUserId, JSON.stringify(routineData));

    // Create 100-day challenge progress (78 days completed)
    for (let day = 1; day <= 78; day++) {
      await db.prepare('INSERT INTO challenge_progress (id, user_id, day, completed) VALUES (?, ?, ?, ?)').run(uuid(), demoUserId, day, 1);
    }
    ...
    for (const entry of moneyEntries) {
      await db.prepare('INSERT INTO money_entries (id, user_id, type, category, amount, description) VALUES (?, ?, ?, ?, ?, ?)').run(uuid(), demoUserId, entry.type, entry.category, entry.amount, entry.desc);
    }
    ...
    for (const cmd of voiceCommands) {
      await db.prepare('INSERT INTO voice_commands (id, user_id, command, response, duration, success) VALUES (?, ?, ?, ?, ?, ?)').run(uuid(), demoUserId, cmd.command, cmd.response, Math.floor(Math.random() * 2000) + 500, 1);
    }
    ...
    for (const goal of abroadGoals) {
      ...
      await db.prepare('INSERT INTO abroad_goals (id, user_id, destination_country, education_level, study_field, intake_month, intake_year, progress, visa_status, days_until_intake) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
    ...
    for (const req of canadaRequirements) {
      await db.prepare('INSERT INTO abroad_requirements (id, goal_id, requirement, priority, status, deadline) VALUES (?, ?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[0], req.requirement, req.priority, req.status, req.deadline);
    }
    ...
    for (const doc of canadaDocuments) {
      await db.prepare('INSERT INTO abroad_documents (id, goal_id, document_type, status, expiry_date) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[0], doc.type, doc.status, doc.expiry);
    }
    ...
    for (const skill of canadaSkills) {
      await db.prepare('INSERT INTO abroad_skills (id, goal_id, skill_name, category, progress_level) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[0], skill.skill, skill.category, skill.level);
    }
    ...
    for (const milestone of canadaTimeline) {
      await db.prepare('INSERT INTO abroad_timeline (id, goal_id, milestone_type, milestone_date, completed) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[0], milestone.type, milestone.date, milestone.completed);
    }
    ...
    for (const exp of canadaExpenses) {
      await db.prepare('INSERT INTO abroad_expenses (id, goal_id, category, amount, paid_status) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[0], exp.category, exp.amount, exp.type);
    }
    ...
    for (const note of canadaNotes) {
      await db.prepare('INSERT INTO abroad_mentor_notes (id, goal_id, category, note) VALUES (?, ?, ?, ?)').run(uuid(), abroadGoalIds[0], note.category, note.note);
    }
    ...
    for (const req of ausRequirements) {
      await db.prepare('INSERT INTO abroad_requirements (id, goal_id, requirement, priority, status, deadline) VALUES (?, ?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[1], req.requirement, req.priority, req.status, req.deadline);
    }
    ...
    for (const doc of ausDocuments) {
      await db.prepare('INSERT INTO abroad_documents (id, goal_id, document_type, status, expiry_date) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[1], doc.type, doc.status, doc.expiry);
    }
    ...
    for (const skill of ausSkills) {
      await db.prepare('INSERT INTO abroad_skills (id, goal_id, skill_name, category, progress_level) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[1], skill.skill, skill.category, skill.level);
    }
    ...
    for (const milestone of ausTimeline) {
      await db.prepare('INSERT INTO abroad_timeline (id, goal_id, milestone_type, milestone_date, completed) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[1], milestone.type, milestone.date, milestone.completed);
    }
    ...
    for (const exp of ausExpenses) {
      await db.prepare('INSERT INTO abroad_expenses (id, goal_id, category, amount, paid_status) VALUES (?, ?, ?, ?, ?)').run(uuid(), abroadGoalIds[1], exp.category, exp.amount, exp.type);
    }
